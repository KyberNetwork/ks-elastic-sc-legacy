...
describe('SwapMath', () => {
  ...
  it('from token0 -> token1 exact input', async () => {
    ...
    const delta = await swapMath.calcDeltaNext(liquidity, priceStart, priceEnd, BN.from(30), true, true);
    console.log(`delta: ${delta.toString()}`); // 0.004995092120267736

    const lc = await swapMath.calcSwapFeeAmounts(delta.sub(ONE), priceStart, BN.from(30), true, true);
  ...
  it('from token0 -> token1 exact output', async () => {
    ...
    const priceEnd = encodePriceSqrt(100, 101);
    // calculate the amount of token0 to swap from 1 -> 1 / 1.01
    const delta = await swapMath.calcDeltaNext(liquidity, priceStart, priceEnd, BN.from(30), false, false);
  ...
  it('from token1 -> token0 exact input', async () => {
    ...
    const delta = await swapMath.calcDeltaNext(liquidity, priceStart, priceEnd, BN.from(30), true, false);
    console.log(`delta: ${delta.toString()}`); // 0.004995092120267736

    const lc = await swapMath.calcSwapFeeAmounts(delta.sub(ONE), priceStart, BN.from(30), true, false);
    console.log(`lc=${lc.toString()}`); // 0.000007492638180401 = 0.004995092120267736 * 0.003 / 2
  ...
  it('from token1 -> token0 exact output', async () => {
    ...
    const delta = await swapMath.calcDeltaNext(liquidity, priceStart, priceEnd, BN.from(30), false, true);
    console.log(`delta: ${delta.toString()}`); // -0.004970250488226176
