...
import {ZERO, ONE, TWO, TWO_POW_96, PRECISION, MAX_UINT, MAX_INT_128} from './helpers/helper';
import chai from 'chai';
const {solidity, loadFixture} = waffle;
chai.use(solidity);

import {MockPosition__factory, MockPosition} from '../typechain';
import {revertToSnapshot, snapshot} from './helpers/hardhat';
...
describe('Position', () => {
  ...
  describe('#update', async () => {
    ...
    it('should be able to initialize first liquidity addition', async () => {
    ...
    it('should be able to correctly update liquidity and feeGrowthInsideLast upon removal', async () => {
      // add liquidity
      await position.update(user.address, tickLower, tickUpper, liquidityDelta, feeGrowthInside);
      // remove liquidity
  ...
  describe('feesClaimable', async () => {
    ...
    it('should test max feeGrowth difference = MAX_UINT * TWO_POW_96 / MAX_INT_128', async () => {
      ...
      expect(await position.feesClaimable()).to.be.gt(MAX_UINT.sub(5));
