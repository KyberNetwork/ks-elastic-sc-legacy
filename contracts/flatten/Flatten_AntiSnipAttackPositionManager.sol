...
// File contracts/periphery/libraries/AntiSnipAttack.sol
  ...
  function update(
    ...
    // scoping of fee proportions to avoid stack too deep
    {
      ...
      uint256 feesClaimableSinceLastActionFeeUnits = Math.min(
        MathConstants.FEE_UNITS,
        (uint256(currentTime - _self.lockTime) * MathConstants.FEE_UNITS) / vestingPeriod
      );
      ...
      uint256 feesClaimableVestedFeeUnits = _self.unlockTime <= _self.lastActionTime
        ...
        : Math.min(
          MathConstants.FEE_UNITS,
          (uint256(currentTime - _self.lastActionTime) * MathConstants.FEE_UNITS) /
      ...
      self.unlockTime = (_self.feesLocked == 0)
        ...
        : (((_self.lockTime + vestingPeriod) *
          feesSinceLastAction *
          (MathConstants.FEE_UNITS - feesClaimableSinceLastActionFeeUnits) +
          _self.unlockTime *
          feesLockedBeforeUpdate *
          (MathConstants.FEE_UNITS - feesClaimableVestedFeeUnits)) / (_self.feesLocked * MathConstants.FEE_UNITS))
        .toUint32();
    }
...
// File contracts/interfaces/periphery/IERC721Permit.sol
...
// File contracts/interfaces/periphery/INonfungibleTokenPositionDescriptor.sol
...
// File contracts/periphery/libraries/LiquidityMath.sol
...
library LiquidityMath {
  ...
  function getLiquidityFromQty0(
    ...
    uint256 liq = FullMath.mulDivFloor(lowerSqrtP, upperSqrtP, MathConstants.TWO_POW_96);
    unchecked {
  ...
  function getLiquidityFromQty1(
    ...
    unchecked {
      return FullMath.mulDivFloor(qty1, MathConstants.TWO_POW_96, upperSqrtP - lowerSqrtP).toUint128();
    }
  }
 ...
 * which allows you to call the safe operations as `token.safeTransfer(...)`, etMathConstants.
 */
library SafeERC20 {
...
// File contracts/periphery/base/ImmutablePeripheryStorage.sol
...
// File contracts/periphery/base/RouterTokenHelper.sol
...
// File contracts/periphery/base/LiquidityHelper.sol
...
// File contracts/interfaces/periphery/IMulticall.sol
...
// File contracts/periphery/base/DeadlineValidation.sol
  ...
  LiquidityHelper
{
    ...
    )
  {
    ...
    if (feeGrowthInsideLast != tmpFeeGrowthInsideLast) {
      ...
      additionalRTokenOwed = FullMath.mulDivFloor(tmpLiquidity, feeGrowthInsideDiff, MathConstants.TWO_POW_96);
    ...
    )
  {
    ...
    if (feeGrowthInsideLast != pos.feeGrowthInsideLast) {
      ...
      (additionalRTokenOwed, ) = AntiSnipAttack.update(
        ...
        true,
        FullMath.mulDivFloor(tmpLiquidity, feeGrowthInsideDiff, MathConstants.TWO_POW_96),
