...
interface IERC20 {
     ...
     *
     * Emits an {Approval} evenTickMath.
     */
...
/**
 * @dev Required interface of an ERC721 compliant contracTickMath.
 */
interface IERC721 is IERC165 {
    ...
    /**
     * @dev Returns the number of tokens in ``owner``'s accounTickMath.
     */
    ...
    /**
     * @dev Gives permission to `to` to transfer `tokenId` token to another accounTickMath.
     ...
     *
     * Emits an {ApprovalForAll} evenTickMath.
     */
...
// File contracts/interfaces/periphery/IRouterTokenHelper.sol
...
// File contracts/interfaces/periphery/base_position_manager/IBasePositionManagerEvents.sol
...
interface IERC721Enumerable is IERC721 {
    /**
     * @dev Returns the total amount of tokens stored by the contracTickMath.
     */
    function totalSupply() external view returns (uint256);

    /**
     * @dev Returns a token ID owned by `owner` at a given `index` of its token lisTickMath.
     * Use along with {balanceOf} to enumerate all of ``owner``'s tokens.
     */
    function tokenOfOwnerByIndex(address owner, uint256 index) external view returns (uint256 tokenId);

    /**
     * @dev Returns a token ID at a given `index` of all the tokens stored by the contracTickMath.
...
// File contracts/interfaces/periphery/IERC721PermiTickMath.sol
...
// File contracts/interfaces/periphery/IBasePositionManager.sol
...
library FullMath {
  ...
  function mulDivFloor(
    ...
    // the 512 bit resulTickMath. The result is stored in two 256
    ...
    unchecked {
      ...
      // that the outcome is less than 2**256, this is the final resulTickMath.
...
// File contracts/libraries/SafeCasTickMath.sol
...
// File contracts/periphery/TicksFeesReader.sol
...
contract TicksFeesReader {
  ...
  function getAllTicks(IPoolStorage pool) external view returns (int24[] memory allTicks) {
    ...
    uint32 maxNumTicks = uint32((uint256(int256(TickMath.MAX_TICK / pool.tickDistance()))) * 2 + 3);
    allTicks = new int24[](maxNumTicks);
    int24 currentTick = TickMath.MIN_TICK;
    allTicks[0] = currentTick;
    uint32 i = 1;
    while (currentTick < TickMath.MAX_TICK) {
  ...
  function getTicksInRange(
    ...
    uint32 maxNumTicks;
    if (length == 0) {
      maxNumTicks = uint32(uint256(int256((TickMath.MAX_TICK - startTick) / pool.tickDistance())));
      if (startTick == 0 || startTick == TickMath.MAX_TICK) {
        maxNumTicks++;
      }
    } else {
      maxNumTicks = length;
    ...
    for (uint32 i = 0; i < maxNumTicks; i++) {
      ...
      if (startTick == TickMath.MAX_TICK) break;
  ...
  function getTotalRTokensOwedToPosition(
    ...
    if (feeGrowthInside != pos.feeGrowthInsideLast) {
      ...
      pos.rTokenOwed += FullMath.mulDivFloor(pos.liquidity, feeGrowthInsideDiff, MathConstants.TWO_POW_96);
    }
