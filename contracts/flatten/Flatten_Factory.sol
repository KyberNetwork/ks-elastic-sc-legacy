// Sources flattened with hardhat v2.8.0-dev.0 https://hardhat.org
...
// File contracts/libraries/CodeDeployer.sol
...
// File contracts/libraries/BaseSplitCodeFactory.sol
 ...
 * which allows you to call the safe operations as `token.safeTransfer(...)`, et MathConstants.
 */
library SafeERC20 {
...
contract ERC20 is Context, IERC20, IERC20Metadata {
     ...
     * e.g. implement automatic token fees, slashing mechanisms, et MathConstants.
     *
     * Emits a {Transfer} event.
     *
     * Requirements:
     ...
     * e.g. set automatic allowances for certain subsystems, et MathConstants.
     *
     * Emits an {Approval} event.
     *
     * Requirements:
...
library QtyDeltaMath {
  ...
  {
    qty0 = FullMath.mulDivCeiling( MathConstants.MIN_LIQUIDITY,  MathConstants.TWO_POW_96, initialSqrtP);
    qty1 = FullMath.mulDivCeiling( MathConstants.MIN_LIQUIDITY, initialSqrtP,  MathConstants.TWO_POW_96);
  }
  ...
  function calcRequiredQty1(
    ...
    unchecked {
      return
        isAddLiquidity
          ? (FullMath.mulDivCeiling(liquidity, upperSqrtP - lowerSqrtP,  MathConstants.TWO_POW_96)).toInt256()
          : (FullMath.mulDivFloor(liquidity, upperSqrtP - lowerSqrtP,  MathConstants.TWO_POW_96)).revToInt256();
    }
  }
  ...
  function getQty0FromBurnRTokens(uint160 sqrtP, uint256 liquidity)
    ...
    returns (uint256)
  {
    return FullMath.mulDivFloor(liquidity,  MathConstants.TWO_POW_96, sqrtP);
  }
  ...
  function getQty1FromBurnRTokens(uint160 sqrtP, uint256 liquidity)
    ...
    returns (uint256)
  {
    return FullMath.mulDivFloor(liquidity, sqrtP,  MathConstants.TWO_POW_96);
  }
...
library SwapMath {
  ...
  function calcReachAmount(
    ...
    if (isExactInput) {
      ...
      if (isToken0) {
        ...
        uint256 denominator =  MathConstants.TWO_FEE_UNITS * targetSqrtP - feeInFeeUnits * currentSqrtP;
        uint256 numerator = FullMath.mulDivFloor(
          liquidity,
           MathConstants.TWO_FEE_UNITS * absPriceDiff,
          denominator
        );
        reachAmount = FullMath.mulDivFloor(numerator,  MathConstants.TWO_POW_96, currentSqrtP).toInt256();
      } else {
        // numerator = 2 * liquidity * absPriceDiff * currentSqrtP
        // denominator = 2 * currentSqrtP - targetSqrtP * feeInFeeUnits / FEE_UNITS
        // overflow should not happen because the absPriceDiff is capped to ~5%
        uint256 denominator =  MathConstants.TWO_FEE_UNITS * currentSqrtP - feeInFeeUnits * targetSqrtP;
        uint256 numerator = FullMath.mulDivFloor(
          liquidity,
           MathConstants.TWO_FEE_UNITS * absPriceDiff,
          denominator
        );
        reachAmount = FullMath.mulDivFloor(numerator, currentSqrtP,  MathConstants.TWO_POW_96).toInt256();
      }
    } else {
      ...
      if (isToken0) {
        ...
        numerator = FullMath.mulDivFloor(liquidity <<  MathConstants.RES_96, numerator, denominator);
        ...
        reachAmount = FullMath.mulDivFloor(numerator, absPriceDiff,  MathConstants.TWO_POW_96).revToInt256();
      }
    }
  }
  ...
  function estimateIncrementalLiquidity(
    ...
    if (isExactInput) {
      if (isToken0) {
        ...
        deltaL = FullMath.mulDivFloor(
          ...
          absDelta * feeInFeeUnits,
           MathConstants.TWO_FEE_UNITS <<  MathConstants.RES_96
        );
      } else {
        // deltaL = feeInFeeUnits * absDelta * / (currentSqrtP * 2)
        ...
        deltaL = FullMath.mulDivFloor(
          ...
          absDelta * feeInFeeUnits,
           MathConstants.TWO_FEE_UNITS * currentSqrtP
        );
      }
    } else {
      ...
      uint256 b = ( MathConstants.FEE_UNITS - feeInFeeUnits) * liquidity;
      ...
      if (isToken0) {
        ...
        b -= FullMath.mulDivFloor( MathConstants.FEE_UNITS * absDelta, currentSqrtP,  MathConstants.TWO_POW_96);
        c = FullMath.mulDivFloor(c, currentSqrtP,  MathConstants.TWO_POW_96);
      } else {
        // a = feeInFeeUnits
        // b = (FEE_UNITS - feeInFeeUnits) * liquidity - FEE_UNITS * absDelta / currentSqrtP
        // c = liquidity * feeInFeeUnits * absDelta / currentSqrtP
        b -= FullMath.mulDivFloor( MathConstants.FEE_UNITS * absDelta,  MathConstants.TWO_POW_96, currentSqrtP);
        c = FullMath.mulDivFloor(c,  MathConstants.TWO_POW_96, currentSqrtP);
      }
  ...
  function calcIncrementalLiquidity(
    ...
    if (isToken0) {
      ...
      // needs to be minimum
      uint256 tmp1 = FullMath.mulDivFloor(liquidity,  MathConstants.TWO_POW_96, currentSqrtP);
      uint256 tmp2 = isExactInput ? tmp1 + absDelta : tmp1 - absDelta;
      uint256 tmp3 = FullMath.mulDivFloor(nextSqrtP, tmp2,  MathConstants.TWO_POW_96);
      ...
      // needs to be minimum
      uint256 tmp1 = FullMath.mulDivFloor(liquidity, currentSqrtP,  MathConstants.TWO_POW_96);
      uint256 tmp2 = isExactInput ? tmp1 + absDelta : tmp1 - absDelta;
      uint256 tmp3 = FullMath.mulDivFloor(tmp2,  MathConstants.TWO_POW_96, nextSqrtP);
      // in edge cases where liquidity or absDelta is small
      // liquidity might be greater than nextSqrtP * ((liquidity / currentSqrtP) +/- absDelta))
      // due to rounding
      deltaL = (tmp3 > liquidity) ? tmp3 - liquidity : 0;
  ...
  function calcFinalPrice(
    ...
    if (isToken0) {
      ...
      uint256 tmp = FullMath.mulDivFloor(absDelta, currentSqrtP,  MathConstants.TWO_POW_96);
      if (isExactInput) {
      ...
      if (isExactInput) {
        uint256 tmp = FullMath.mulDivFloor(absDelta,  MathConstants.TWO_POW_96, currentSqrtP);
  ...
  function calcReturnedAmount(
    ...
    if (isToken0) {
      if (isExactInput) {
        ...
        returnedAmount =
          FullMath.mulDivCeiling(deltaL, nextSqrtP,  MathConstants.TWO_POW_96).toInt256() +
          FullMath.mulDivFloor(liquidity, currentSqrtP - nextSqrtP,  MathConstants.TWO_POW_96).revToInt256();
      } else {
        // maximise actual input (>0) so we get desired output amount
        // returnedAmount = deltaL * nextSqrtP + liquidity * (nextSqrtP - currentSqrtP)
        returnedAmount =
          FullMath.mulDivCeiling(deltaL, nextSqrtP,  MathConstants.TWO_POW_96).toInt256() +
          FullMath.mulDivCeiling(liquidity, nextSqrtP - currentSqrtP,  MathConstants.TWO_POW_96).toInt256();
      }
    } else {
      ...
      returnedAmount =
        FullMath.mulDivCeiling(liquidity + deltaL,  MathConstants.TWO_POW_96, nextSqrtP).toInt256() +
        FullMath.mulDivFloor(liquidity,  MathConstants.TWO_POW_96, currentSqrtP).revToInt256();
    }
...
abstract contract PoolStorage is IPoolStorage {
  ...
  function _initPoolStorage(uint160 initialSqrtP, int24 initialTick) internal {
    poolData.baseL = 0;
    poolData.reinvestL =  MathConstants.MIN_LIQUIDITY;
    poolData.reinvestLLast =  MathConstants.MIN_LIQUIDITY;
...
contract Pool is IPool, PoolTicksState, ERC20('KyberSwap v2 Reinvestment Token', 'KS2-RT') {
    ...
    returns (uint256 qty0, uint256 qty1)
  {
    ...
    _mint(address(this),  MathConstants.MIN_LIQUIDITY);
  ...
  function swap(
    ...
    while (swapData.specifiedAmount != 0 && swapData.sqrtP != limitSqrtP) {
      ...
      if (willUpTick && tempNextTick >  MathConstants.MAX_TICK_DISTANCE + swapData.currentTick) {
        tempNextTick = swapData.currentTick +  MathConstants.MAX_TICK_DISTANCE;
      } else if (!willUpTick && tempNextTick < swapData.currentTick -  MathConstants.MAX_TICK_DISTANCE) {
        tempNextTick = swapData.currentTick -  MathConstants.MAX_TICK_DISTANCE;
      }
      ...
      if (rMintQty != 0) {
        ...
        unchecked {
          uint256 governmentFee = (rMintQty * cache.governmentFeeUnits) /  MathConstants.FEE_UNITS;
          cache.governmentFee += governmentFee;

          uint256 lpFee = rMintQty - governmentFee;
          cache.lpFee += lpFee;

          cache.feeGrowthGlobal += FullMath.mulDivFloor(lpFee,  MathConstants.TWO_POW_96, swapData.baseL);
        }
      }
  ...
  function flash(
    ...
    uint256 feeQty1;
    if (feeTo != address(0)) {
      feeQty0 = (qty0 * swapFeeUnits) /  MathConstants.FEE_UNITS;
      feeQty1 = (qty1 * swapFeeUnits) /  MathConstants.FEE_UNITS;
    }
    uint256 balance0Before = _poolBalToken0();
    uint256 balance1Before = _poolBalToken1();
    ...
    returns (uint128)
  {
    ...
    if (secondsElapsed > 0 && baseL > 0) {
      _secondsPerLiquidityGlobal += uint128((secondsElapsed <<  MathConstants.RES_96) / baseL);
      // write to storage
  ...
  function _syncFeeGrowth(
    ...
    if (rMintQty != 0) {
      ...
      unchecked {
        _feeGrowthGlobal += FullMath.mulDivFloor(rMintQty,  MathConstants.TWO_POW_96, baseL);
      }
